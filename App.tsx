...
export default function App() {
  ...
  const suggestions = useMemo(() => query.trim() && !hasSearched ? searchGuests(guests, query).slice(0, 6) : [], [guests, hasSearched, query]);
  ...
  return (
    <SafeAreaView style={s.page}>
      ...
      <View style={s.frame}>
        ...
        {screen === 'search' ? (
          <SearchScreen query={query} setQuery={(v) => { setQuery(v); setHasSearched(false); }} searched={hasSearched} results={results} suggestions={suggestions} onSearch={() => { Keyboard.dismiss(); setHasSearched(true); }} onSelectSuggestion={(guest) => { setQuery(guest.name); setHasSearched(true); Keyboard.dismiss(); }} onClear={() => { setQuery(''); setHasSearched(false); }} onEdit={setEditorGuest} onOpenMap={setMapGuest} onToggleCheckIn={(guest) => void toggleCheckIn(guest)} onAdd={() => setEditorGuest(null)} />
        ) : screen === 'guests' ? (
...
type SearchProps = { query: string; setQuery: (v: string) => void; searched: boolean; results: Guest[]; suggestions: Guest[]; onSearch: () => void; onSelectSuggestion: (g: Guest) => void; onClear: () => void; onEdit: (g: Guest) => void; onOpenMap: (g: Guest) => void; onToggleCheckIn: (g: Guest) => void; onAdd: () => void };
function SearchScreen({ query, setQuery, searched, results, suggestions, onSearch, onSelectSuggestion, onClear, onEdit, onOpenMap, onToggleCheckIn, onAdd }: SearchProps) {
  ...
  return <View style={s.screen}><FlatList data={results} keyExtractor={(item) => item.id} contentContainerStyle={s.content} keyboardDismissMode="on-drag" keyboardShouldPersistTaps="handled" initialNumToRender={8} maxToRenderPerBatch={8} removeClippedSubviews={Platform.OS === 'android'} updateCellsBatchingPeriod={40} windowSize={7}
    ListHeaderComponent={<><Text style={s.title}>Encontre um convidado</Text><Text style={s.subtitle}>Digite o nome ou a função para consultar a mesa e o grupo.</Text><View style={[s.searchRow, compact && s.searchRowCompact]}><View style={s.searchBox}><Text style={s.searchGlyph}>⌕</Text><TextInput accessibilityLabel="Nome ou função do convidado" autoCapitalize="words" autoCorrect={false} onChangeText={setQuery} onSubmitEditing={onSearch} placeholder="Nome ou função" placeholderTextColor="#899188" returnKeyType="search" style={s.searchInput} value={query} />{!!query && <Pressable accessibilityLabel="Limpar busca" onPress={onClear} style={s.clear}><Text style={s.clearText}>×</Text></Pressable>}</View><Button label="Buscar" onPress={onSearch} compact disabled={!query.trim()} /></View>{!searched && !!suggestions.length && <View style={s.suggestions}><Text style={s.suggestionsTitle}>SUGESTÕES</Text>{suggestions.map((guest) => <Pressable accessibilityLabel={`Selecionar ${guest.name}, mesa ${guest.table}`} accessibilityRole="button" key={guest.id} onPress={() => onSelectSuggestion(guest)} style={({ pressed }) => [s.suggestionRow, pressed && s.suggestionPressed]}><View style={s.suggestionAvatar}><Text style={s.suggestionAvatarText}>{guest.name.charAt(0).toUpperCase()}</Text></View><View style={s.suggestionBody}><Text numberOfLines={1} style={s.suggestionName}>{guest.name}</Text><Text numberOfLines={1} style={s.suggestionRole}>{guest.role}</Text></View><View style={s.suggestionTable}><Text style={s.suggestionTableLabel}>MESA</Text><Text style={s.suggestionTableNumber}>{guest.table}</Text></View></Pressable>)}</View>}{!searched && !!query.trim() && !suggestions.length && <Text style={s.noSuggestions}>Nenhuma sugestão encontrada.</Text>}{searched && <Text style={s.resultCount}>{results.length === 1 ? '1 resultado encontrado' : `${results.length} resultados encontrados`}</Text>}</>}
    renderItem={({ item }) => <ResultCard guest={item} onEdit={() => onEdit(item)} onOpenMap={() => onOpenMap(item)} onToggleCheckIn={() => onToggleCheckIn(item)} />}
    ListEmptyComponent={searched ? <Empty title="Nenhum convidado encontrado" text="Confira o nome ou a função, ou cadastre esta pessoa na lista." action="Cadastrar convidado" onPress={onAdd} /> : query.trim() ? null : <View style={s.welcome}><Text style={s.welcomeKicker}>CONSULTA RÁPIDA</Text><Text style={s.welcomeTitle}>A mesa certa, sem demora.</Text><Text style={s.welcomeText}>A busca encontra partes do nome ou da função e ignora diferenças de acentos e letras maiúsculas.</Text></View>}
  /></View>;
}
...
const s = StyleSheet.create({
  ...
  searchRow: { flexDirection: 'row', gap: 10, marginTop: 22 }, searchRowCompact: { alignItems: 'stretch', flexDirection: 'column' }, searchBox: { flex: 1, minHeight: 52, backgroundColor: C.card, borderColor: C.line, borderRadius: 13, borderWidth: 1, flexDirection: 'row', alignItems: 'center' }, searchGlyph: { color: C.green, fontSize: 25, marginLeft: 14 }, searchInput: { flex: 1, color: C.ink, fontSize: 16, paddingHorizontal: 10, paddingVertical: 13 }, clear: { height: 40, width: 40, alignItems: 'center', justifyContent: 'center' }, clearText: { color: C.muted, fontSize: 25 }, suggestions: { backgroundColor: C.card, borderColor: C.line, borderRadius: 13, borderWidth: 1, marginTop: 8, overflow: 'hidden' }, suggestionsTitle: { color: C.muted, fontSize: 10, fontWeight: '800', letterSpacing: 1.2, paddingBottom: 9, paddingHorizontal: 14, paddingTop: 11 }, suggestionRow: { alignItems: 'center', borderTopColor: C.line, borderTopWidth: 1, flexDirection: 'row', minHeight: 62, paddingHorizontal: 12, paddingVertical: 8 }, suggestionPressed: { backgroundColor: C.greenSoft }, suggestionAvatar: { alignItems: 'center', backgroundColor: C.greenSoft, borderRadius: 18, height: 36, justifyContent: 'center', width: 36 }, suggestionAvatarText: { color: C.green, fontSize: 14, fontWeight: '800' }, suggestionBody: { flex: 1, marginLeft: 10, minWidth: 0 }, suggestionName: { color: C.ink, fontSize: 14, fontWeight: '700' }, suggestionRole: { color: C.gold, fontSize: 11, fontWeight: '700', marginTop: 2 }, suggestionTable: { alignItems: 'center', marginLeft: 10, minWidth: 45 }, suggestionTableLabel: { color: C.muted, fontSize: 8, fontWeight: '800', letterSpacing: 0.8 }, suggestionTableNumber: { color: C.green, fontSize: 16, fontWeight: '900', marginTop: 1 }, noSuggestions: { color: C.muted, fontSize: 12, marginTop: 10, textAlign: 'center' },
