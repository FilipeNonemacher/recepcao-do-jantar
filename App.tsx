...
export default function App() {
  ...
  const checkedInPeople = guests.reduce((sum, guest) => sum + (guest.checkedIn ? groupSize(guest) : 0), 0);
    ...
    <SafeAreaView style={s.page}>
      ...
      <View style={s.frame}>
        ...
        {screen === 'search' ? (
          ...
          <GuestsScreen guests={searchGuests(guests, '')} onAdd={() => setEditorGuest(null)} onEdit={setEditorGuest} onDelete={removeGuest} onToggleCheckIn={(guest) => void toggleCheckIn(guest)} />
        ) : screen === 'occupancy' ? (
...
function Header({ guests, people, present }: { guests: number; people: number; present: number }) {
  return <View style={s.header}><View><Text style={s.eyebrow}>RECEPÇÃO</Text><Text style={s.brand}>Jantar</Text></View><View style={s.stats}><Stat value={guests} label="grupos" /><View style={s.statDivider} /><Stat value={people} label="previstas" /><View style={s.statDivider} /><Stat value={present} label="presentes" /></View></View>;
}
...
function GuestsScreen({ guests, onAdd, onEdit, onDelete, onToggleCheckIn }: { guests: Guest[]; onAdd: () => void; onEdit: (g: Guest) => void; onDelete: (g: Guest) => void; onToggleCheckIn: (g: Guest) => void }) {
  return <View style={s.screen}><FlatList data={guests} keyExtractor={(item) => item.id} contentContainerStyle={s.guestContent}
    ...
    renderItem={({ item }) => <View style={s.listCard}><View style={[s.avatar, item.checkedIn && s.avatarChecked]}><Text style={[s.avatarText, item.checkedIn && s.avatarTextChecked]}>{item.checkedIn ? '✓' : item.name.charAt(0).toUpperCase()}</Text></View><View style={s.listBody}><Text style={s.listName}>{item.name}</Text><Text style={s.listRole}>{item.role}</Text><Text style={s.listMeta}>Mesa {item.table} · {item.companions} {item.companions === 1 ? 'acompanhante' : 'acompanhantes'} · {item.checkedIn ? 'presente' : 'aguardando'}</Text></View><Pressable accessibilityLabel={`${item.checkedIn ? 'Desfazer check-in de' : 'Fazer check-in de'} ${item.name}`} onPress={() => onToggleCheckIn(item)} style={[s.iconButton, item.checkedIn && s.checkedButton]}><Text style={[s.iconText, item.checkedIn && s.checkedButtonText]}>✓</Text></Pressable><Pressable accessibilityLabel={`Editar ${item.name}`} onPress={() => onEdit(item)} style={s.iconButton}><Text style={s.iconText}>✎</Text></Pressable><Pressable accessibilityLabel={`Excluir ${item.name}`} onPress={() => onDelete(item)} style={[s.iconButton, s.deleteButton]}><Text style={s.deleteText}>×</Text></Pressable></View>}
